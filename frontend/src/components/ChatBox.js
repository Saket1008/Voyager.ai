...
// --- ChatGPT-style Clone (React + Tailwind) --- //
// Features:
// • Glassy sticky header with sidebar toggle, centered title, actions (New, Settings)
// • Collapsible left sidebar with New Chat + Recent chats
// • Chat area with user/assistant bubbles, hover actions (Copy, options)
// • Typing indicator (three bouncing dots) + character-by-character streaming
// • Regenerate & Stop generating controls on assistant messages
// • Sticky input bar, Enter-to-send, Shift+Enter for new line
...
export default function ChatGPTClone() {
  ...
  function pushMessage(role, text) {
    setChats((prev) =>
      prev.map((c) =>
        c.id === activeId
          ? { ...c, messages: [...c.messages, { id: cryptoRandomId(), role, text }] }
          : c
      )
    );
  }
  ...
  function stopGenerating() {
    ...
    if (streamBufferRef.current) {
      // Commit partial buffer as assistant message
  ...
  function regenerate() {
    ...
    if (lastUser) simulateStreamReply(lastUser.text);
  }

  // Simulated streaming response (replace with real API later)
  function simulateStreamReply(prompt) {
    stopGenerating(); // ensure no parallel streams
    setIsTyping(true);
    const reply = generateAssistantText(prompt);
    streamBufferRef.current = "";
    ...
    intervalRef.current = window.setInterval(() => {
      ...
      i++;
      // Live preview by replacing/adding temp streaming row
      setChats((prev) =>
        prev.map((c) => {
          if (c.id !== activeId) return c;
          const msgs = [...c.messages];
          const last = msgs[msgs.length - 1];
          // Show ephemeral streaming bubble by storing it nowhere; just re-render based on buffer below
  ...
  function MessageBubble({ m }) {
    ...
    return (
      <motion.div
        ...
        className={`group relative w-fit max-w-[85%] rounded-xl px-4 py-3 text-sm leading-relaxed shadow-sm transition ${
          isUser
      ...
      >
        ...
        {!isUser && !isWizardCommand && (
          <div className="absolute -right-10 top-1/2 hidden -translate-y-1/2 items-center gap-2 group-hover:flex">
            ...
            <IconButton onClick={() => {}} ariaLabel="More options">
        ...
        <div className="min-h-[1lh] whitespace-pre-wrap">
          {streamBufferRef.current || (
            <div className="flex items-center gap-2 text-gray-300">
              <TypingDots /> <span className="text-xs">Voyager.AI is crafting your journey...</span>
            </div>
          )}
        </div>
        <div className="mt-2 flex gap-2">
  ...
  return (
    <motion.div 
      className="flex h-screen w-full select-none text-white"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      <motion.aside
        ...
        } hidden md:flex h-full flex-col overflow-hidden border-r border-white/20 bg-black/40 backdrop-blur-xl transition-[width] duration-300`}
      ...
      >
        <div className="p-3 border-b border-white/20 flex items-center justify-between">
          <button
            className="w-full rounded-lg border border-white/20 bg-black/60 backdrop-blur-sm px-3 py-2 text-sm text-white hover:bg-black/80 transition-colors"
            onClick={newChat}
          >
            + New Journey
          </button>
          ...
          {chats.map((c) => (
            <div
              ...
              className={`group flex items-center justify-between rounded-lg px-3 py-2 text-sm cursor-pointer ${
                c.id === activeId ? "bg-white/10" : "hover:bg-white/10"
              }`}
              onClick={() => setActiveId(c.id)}
            >
              <div className="truncate text-white" title={c.title}>
                {c.title}
              </div>
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition">
                <IconButton onClick={() => deleteChat(c.id)} ariaLabel="Delete chat">
                  <Trash2 size={14} />
                </IconButton>
              </div>
            </div>
      ...
      {/* Main column */}
      <motion.div 
        className="flex h-full flex-1 flex-col bg-black/40 backdrop-blur-xl"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        ...
        <header className="sticky top-0 z-10 border-b border-white/20 bg-black/60 backdrop-blur-xl">
          <div className="mx-auto flex h-14 max-w-5xl items-center gap-3 px-3">
            ...
            <div className="pointer-events-none absolute left-1/2 -translate-x-1/2 text-sm font-medium text-white">
              Voyager.AI
            </div>

            <div className="ml-auto flex items-center gap-2">
              <IconButton onClick={newChat} ariaLabel="New chat">
                <Plus size={18} />
              </IconButton>
              <IconButton onClick={() => {}} ariaLabel="Settings">
                <Settings size={18} />
              </IconButton>
            </div>
          </div>
          ...
          >
            <div className="rounded-xl border border-white/20 bg-black/60 backdrop-blur-sm p-2 shadow-lg">
              ...
              <div className="flex items-center justify-between px-2">
                ...
                <button
                  onClick={send}
                  className="rounded-lg bg-[#19c37d] px-3 py-1.5 text-xs font-medium text-white hover:brightness-105 active:translate-y-[1px]"
                >
...
function IconButton({ children, onClick, ariaLabel, className = "" }) {
  return (
    <button
      ...
      onClick={onClick}
      className={`grid h-9 w-9 place-items-center rounded-lg border border-white/20 bg-black/60 backdrop-blur-sm hover:bg-black/80 active:translate-y-[1px] text-white ${className}`}
    >
      {children}
    </button>
  );
...
function SmallButton({ children, onClick, icon }) {
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-1.5 rounded-lg border border-white/20 bg-black/60 backdrop-blur-sm px-2.5 py-1 text-xs text-white hover:bg-black/80 transition-colors"
    >
      {icon}
      {children}
    </button>
